...
function initButtons() {
  ...
  generateTunetableBtn.addEventListener('click', () => {
  ...
  clearTunetableBtn.addEventListener('click', () => {
  ...
  sortTunetableBtn.addEventListener('click', () => {

    if (checkIfEmptyJson(customJson)) {

      infoBox.innerHTML = "No tune data found! Insert link to The Session and press Generate.";
      return;
    }
  ...
  for (let k = 0; k < 4; k++) {
    radioBtn[k].addEventListener("change", function() {
      infoBox.innerHTML = `Sorting style #${this.value}`;
